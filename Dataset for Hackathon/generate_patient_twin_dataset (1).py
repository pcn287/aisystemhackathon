...
# Date range: current date and last 1 year
...
def generate_appointments(patients_df):
    ...
    for i in range(1, N_APPOINTMENTS + 1):
        ...
        # Appointments in last 1 year
        appointment_date = ONE_YEAR_AGO + timedelta(days=random.randint(0, 365))
...
def generate_icu_beds(admissions_df, patients_df):
    ...
    for i in range(1, N_ICU_BEDS + 1):
        ...
        expected_discharge = (TODAY + timedelta(days=random.randint(0, 7))).strftime("%Y-%m-%d") if occupied else None
...
def generate_risk_scores(patients_df, appointments_df):
    ...
    for _, row in patients_df.iterrows():
        ...
        risk_scores.append({
            ...
            "risk_last_updated": TODAY.strftime("%Y-%m-%d %H:%M:%S"),
        })
